...
struct PlayerListView: View {
    ...
    var body: some View {
        ...
        .alert(isPresented: $showDeleteAlert) {
            Alert(
                title: Text("player.remove".localized()),
                message: Text(String(format: "player.remove.confirm".localized(), playerToDelete?.name ?? "")),
                primaryButton: .destructive(Text("player.remove".localized())) {
                    if let player = playerToDelete {
                        _ = playerListViewModel.deletePlayer(byID: player.id)
                    }
                    playerToDelete = nil
                },
                secondaryButton: .cancel(Text("alert.button.cancel".localized())) {
                    playerToDelete = nil
                }
            )
        }
